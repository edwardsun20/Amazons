...
class BoardWidget extends Pad {
    ...
    private void drawGrid(Graphics2D g) {
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 10; j++) {
                ...
                g.fillRect(i * SQUARE_SIDE,
                        j * SQUARE_SIDE, SQUARE_SIDE, SQUARE_SIDE);
            }
        }
    }
    ...
    public synchronized void paintComponent(Graphics2D g) {
        drawGrid(g);
        for (Map.Entry<Square, Piece> entry : _board.map().entrySet()) {
            Square square = entry.getKey();
            Piece piece = entry.getValue();
            if (piece.equals(SPEAR)) {
                g.setColor(SPEAR_COLOR);
                g.fillRect(cx(square.col()), cy(square.row()),
                        SQUARE_SIDE, SQUARE_SIDE);
                continue;
            }
            drawQueen(g, square, piece);
        }
    }
